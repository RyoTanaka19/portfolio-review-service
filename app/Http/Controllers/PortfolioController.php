...
class PortfolioController extends Controller
{
    ...
    public function store(PortfolioRequest $request)
    {
        ...
        $imagePath = $request->hasFile('image')
            ? $request->file('image')->store('portfolios', 's3') // ← public → s3
            : null;
        ...
        return Inertia::render('Portfolios/Show', [
            'portfolio' => [
                ...
                'image_url' => $portfolio->image_path 
                    ? Storage::disk('s3')->url($portfolio->image_path) // ← s3に変更
                    : null,
    ...
    public function update(PortfolioRequest $request, Portfolio $portfolio)
    {
        ...
        if ($request->file('image')) {
            if ($portfolio->image_path) {
                Storage::disk('s3')->delete($portfolio->image_path); // ← public → s3
            }
            $portfolio->image_path = $request->file('image')->store('portfolios', 's3'); // ← public → s3
            $portfolio->save();
